...
Feature: 啟動旅程
  ...
  Rule: 後置（回應） - 系統應在訪客停止捲動 1 秒後重新顯示繼續捲動提示直到點題卡進入畫面
    ...
    @happy
    Scenario: 片尾停下的訪客仍收到繼續捲動提示
      Given 訪客 "Visitor001" 已捲至片尾字卡且點題卡尚未進入畫面
      When "Visitor001" 超過 1 秒未捲動
      Then 畫面下方應重新顯示繼續捲動提示
    ...
    @boundary
    Scenario: 點題卡進入畫面後不再提示
      Given 點題卡已進入 "Visitor001" 的畫面
      When "Visitor001" 停止捲動超過 1 秒
      Then 繼續捲動提示維持不顯示
