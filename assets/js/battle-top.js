(() => {
  ...
  // ---------- 零件系統（v3）：獎勵是真的零件——上蓋 / 固鎖 / 軸心，每層只能裝一個 ----------
  // 上蓋固定不換（它就是這顆陀螺）；能換的只有固鎖與軸心
  const partSlots = [
    { key: 'blade', label: '上蓋', english: 'Blade', rate: 0, swap: false },
    { key: 'ratchet', label: '固鎖', english: 'Ratchet', rate: .5, swap: true },
  ...
  // 三種零件各自長得不一樣：上蓋 = 官方商品圖（上蓋就是陀螺的外觀）；固鎖 = 有棘齒環的塑膠齒輪，齒數 = 名字前面的數字、高度 = 後面的數字；軸心 = 側視的底座 + 軸尖形狀
  ...
  // 一場是一連串事件：被撞進洞的還有機會彈回場內，所以分數會累加；有齒輪軸心的可以咬上藍色齒緣（X 軌道）衝刺再撞
  ...
  // 齒輪軸心（Gear / Accel / Rush，或 X 衝刺 ≥ 40）才咬得上藍色齒緣
  ...
  function simulateKnockoutBattle(player, enemy) {
    ...
    while (points.player < TARGET && points.enemy < TARGET && round < 8) {
      round += 1;
      // 齒輪軸心幾乎每回合都會上軌道；沒人擲中的話，第一回合由 X 衝刺較高的一方上，保證每場至少看得到一次
  ...
  function buildCustomTop() {
    ...
    return {
      ...
      skill: equipped.length ? `已換裝：${equipped.map(item => `${slotMeta(item.instance.slot).label} ${item.part.name}`).join('、')}。` : '打贏對手會掉零件，再回來換固鎖或軸心。'
    };
  }
  ...
  function startCustomizing(productId = '') {
    ...
    els.customStatus.textContent = seed.isCustom ? '點左邊的固鎖 / 軸心，右邊挑零件換上去。改名字會自動儲存。' : '打贏拿到的零件在這裡換上去：固鎖、軸心各一個，上蓋固定。';
