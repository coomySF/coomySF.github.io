(() => {
  ...
  function buildScene() {
    ...
    // 藍線輪廓：大圓 + 上緣兩個飽滿往外凸的圓弧 + 中間方形缺口（直壁、水平平底）。所有接縫都是零斜率的平滑曲線，沒有角
    const rad = deg => deg * Math.PI / 180;
    const norm = deg => ((deg + 540) % 360) - 180;
    const smooth = t => { const k = Math.max(0, Math.min(1, t)); return k * k * (3 - 2 * k); };
    const TOP = { start: 190, wall: 257, wallEnd: 261, endR: .80, bulge: .085, floor: .6015 };
    const radial = theta => {
      const t = ((theta % 360) + 360) % 360;
      const a = 270 - Math.abs(norm(t - 270));            // 以正上方為軸鏡射：a 從 180（側邊）走到 270（正上方）
      if (a <= TOP.start) return 1;
      const floorR = TOP.floor / Math.abs(Math.sin(rad(a)));   // 平底是一條水平線
      if (a >= TOP.wallEnd) return floorR;
      if (a >= TOP.wall) return TOP.endR - (TOP.endR - floorR) * smooth((a - TOP.wall) / (TOP.wallEnd - TOP.wall));
      const u = (a - TOP.start) / (TOP.wall - TOP.start);
      const base = 1 - (1 - TOP.endR) * smooth((u - .3) / .7);          // 前段貼著大圓，後段緩緩降到缺口壁頂
      const bump = TOP.bulge * Math.pow(Math.sin(Math.PI * u), 1.8);     // 往外凸的圓弧，兩端斜率為零
      return base + bump;
    };
